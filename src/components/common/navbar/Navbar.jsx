...
//               <span className="absolute -top-2 -right-2 bg-primary text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center">
//                 0
//               </span>
//             </Link>
...
import { useCurrency } from "@/context/CurrencyContext";
import { useProducts } from "@/context/ProductContext";
import { useCart } from "@/context/CartContext"; // ১. Cart Context ইম্পোর্ট করুন
import DesktopMenu from "./DesktopMenu";
import Sidebar from "./Sidebar";
import TopBar from "./TopBar";
...
const Navbar = () => {
  ...
  const { currency, setCurrency } = useCurrency();
  const { categories, products } = useProducts();
  const { cartCount } = useCart(); // ২. cartCount বের করে নিন
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4 text-secondary">
            <Link href="/cart" className="relative hover:text-primary">
              ...
              {/* ৩. স্ট্যাটিক ০ এর জায়গায় cartCount ব্যবহার করুন এবং ০ হলে হাইড করে দিন */}
              {cartCount > 0 && (
